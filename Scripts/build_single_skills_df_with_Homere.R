...
library(rvest)
library(writexl)
library(xlsx) # just added this new comment #
#
...
#
# Prepare the Homere Data Frame

OCG_Hom_Sk_df_single <- data.frame(UniqueID = character(),
                                   Skill = character(),
                                   stringsAsFactors = FALSE)

nu_row = nrow(OCG_Homere_Skills)


# para OCG_Hom_Sk_df_single  Creo que genero filas vacias eliminalas luego
nu_roww = 1
for(i in 1: nu_row){
  if(OCG_Homere_Skills[i,3] == "Y"){
    OCG_Hom_Sk_df_single[nu_roww,1] =  OCG_Homere_Skills[i,1]
    OCG_Hom_Sk_df_single$Skill[nu_roww] = "English"
  }
  nu_roww = nu_roww+1
  if(OCG_Homere_Skills[i,4] == "Y"){
    OCG_Hom_Sk_df_single[nu_roww,1] =  OCG_Homere_Skills[i,1]
    OCG_Hom_Sk_df_single$Skill[nu_roww] = "French"
  }
  nu_roww = nu_roww+1
  if(OCG_Homere_Skills[i,5] == "Y"){
    OCG_Hom_Sk_df_single[nu_roww,1] =  OCG_Homere_Skills[i,1]
    OCG_Hom_Sk_df_single$Skill[nu_roww] = "Spanish"
  }
  nu_roww = nu_roww+1
  if(!(is.na(OCG_Homere_Skills[i,6]))) {
    OCG_Hom_Sk_df_single[nu_roww,1] =  OCG_Homere_Skills[i,1]
    OCG_Hom_Sk_df_single$Skill[nu_roww] = OCG_Homere_Skills[i,6]
  }
  nu_roww = nu_roww+1
}

OCG_Hom_Sk_df_single <- na.omit(OCG_Hom_Sk_df_single)

# Filas en Repo_Skills se controlan por estas variables 
# Homere needs more control is set last
low_H = nrow(REPO_SKILLS) + 1 # Next available position
top_H = nrow(OCG_Hom_Sk_df_single) - 1 # Max number of records to be added
# OCG_Hom_row = nrow(OCG_Homere_Skills)
# delta_r = top_H - OCG_Hom_row
#skill_a = " "
level_a = " "
level_t = " "
top_iter = top_H + low_H
for(i in low_H : top_iter){
  delta_r = as.integer(i - low_H +1)
  REPO_SKILLS[i,1] = "OCG_Homere"
  REPO_SKILLS[i,2] = OCG_Hom_Sk_df_single[delta_r,1] # Homere UniqueID
  REPO_SKILLS[i,3] = OCG_Hom_Sk_df_single[delta_r,2] # Homere Skill
