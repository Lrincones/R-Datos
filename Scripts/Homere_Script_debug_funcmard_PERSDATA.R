...
prepare_offc_IDS <- function(office_file , ID_datos, data_fram_1, col_param, old_repo_id) { # each file is from an office and group data
  ...
  # it is OCG Office as per parameter ID_datos column 1
  ...
  check_system = ID_datos$System[1] %in% system_loaded # if false load all case 1 first time load
  # Case 1 first time loaded
    while (n_row  < top_row & !check_office) { # first load while there are records to be read load them
      ...
      if(check_office == FALSE || check_system == FALSE){ # if the record does not exits process it 
...
while (arch_read <= num_arch){
  ...
  # Falta revisar que la clave exista
  # col_ids = repo_id_old[,col_param] 
  # col_ids_v = unlist(col_ids)
  # check_record = offc_datos[n_row, col_param] %in% col_ids_v # check if record is in repository
  # if(check_record == FALSE){ # if the record does not exits process it
  # Falta actualizar con lo que se haya leido en la carga de datos
  # esto en a proxima version
  n_row  = 1 # to control records read from office_file
  ...
  # OCG Records Homere
  #
  # col_ofc_OCG <- repo_id_old[which(repo_id_old$office == "OCG")] 
  ...
  #
  # IDS to check if record was loaded  OJO se esta haciendo en  col_ofc_OCG_V son los de persdata
  #
