...
library(lubridate)
library(hablar)
# Start with Canada Personal Data
...
# function to prepare the file for the office ID loaded example "CANISHeroPersonalData"
...
path_1 <-  "C:/Users/luis.rincones/OneDrive - MSF/Documents/0-DataRep/0-May_25-Repository/Input/"
archivos <- list.files(path = path_1,recursive = F , 
                       all.files = FALSE, full.names = TRUE, include.dirs = FALSE) #  Check lower and upper case for testing I used ",pattern = "*Can*""
...
while (arch_read <= num_arch){
  ...
  repo_id_old <- read_excel(
    "C:/Users/luis.rincones/OneDrive - MSF/Documents/0-DataRep/0-May_25-Repository/NoInput/REPO_IDS.xlsx"
  ...
  Loaded_Summary_Old =read_excel(
    "C:/Users/luis.rincones/OneDrive - MSF/Documents/0-DataRep/0-May_25-Repository/NoInput/Loaded_Summary.xlsx"
     ) # creating  need to convert numeric field to character
  ...
  df_IDS <- read_excel(
    "C:/Users/luis.rincones/OneDrive - MSF/Documents/0-DataRep/0-May_25-Repository/NoInput/df_id_template.xlsx"
  ...
  ID_filewr = paste("C:/Users/luis.rincones/OneDrive - MSF/Documents/0-DataRep/0-May_25-Repository/NoInput/", ID_filewr, sep = "")
  ...
  # delete after test communicaions load
  ...
  while (n_row <= top_row) {
    # the legacyid for the record
    #off_id = offc_datos[n_row, c] # usa off_c
    # repo_v has a single value with the corresponding repoid from the perdata loaded for the office to use in the new repo_id
    # for PersonalData records 
    check_record = offc_datos[n_row, c] %in% col_ids_leg_repo_v # check if record legacy id is in repository if it is process it perdata loaded
    if(check_office){
     
      repo_v <- subset(repo_id_old, repo_id_old$office == off_c & repo_id_old$source == "PersonalData"
                       & offc_datos[n_row,c] %in% repo_id_old$Legacy_ID, 
                       select = c(repoid, Legacy_ID))
      repo_IV <- subset(repo_v, repo_v$Legacy_ID %in% offc_datos[n_row,c], select = c(repoid))
      
    
      
      if(!check_record){ # SKIP record
        n_row = n_row + 1 # Skip record but do not skip the record to write
        ...
        repo_id_old[n_roww + last_key,'repoid'] = repo_IV
        repo_id_old[n_roww + last_key,'Legacy_ID'] = offc_datos[n_row,c] # cual columna
        n_row = n_row + 1
        n_roww = n_roww + 1
      }
    }
      ...
      repo_id_old[n_roww + last_key,'system'] = offc_datos_id[3]
      repo_id_old[n_roww + last_key,'key'] = offc_datos_id[4]
      repo_id_old[n_roww + last_key,'repoid'] = as.character(last_key_val + n_roww)
      repo_id_old[n_roww + last_key,'Legacy_ID'] = offc_datos[n_row,c] # cual columna
      n_row = n_row + 1
      n_roww = n_roww + 1
    }
  ...
  write_xlsx(repo_id_old, "C:/Users/luis.rincones/OneDrive - MSF/Documents/0-DataRep/0-May_25-Repository/NoInput/REPO_IDS.xlsx", 
  ...
  ID_filewrd1 = paste("C:/Users/luis.rincones/OneDrive - MSF/Documents/0-DataRep/0-May_25-Repository/NoInput/", ID_filewrd1, sep = "")
...
setwd("C:/Users/luis.rincones/OneDrive - MSF/Documents/0-DataRep/0-May_25-Repository/NoInput/")
