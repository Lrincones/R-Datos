# Personal Data Hero and Dynamics -Canada,Italy and OCG
...
# function to prepare the file for the office ID loaded example "CANHeroPersonalData"
...
# recall that the position depends on the raw data, the position for the ID column in the data varies from source
# using hard code 4, next version use a parameter from the function call. 
#
...
prepare_offc_IDS <- function(office_file , ID_datos, data_fram_1, col_param, old_repo_id) { # each file is from an office and group data
  ...
  # Which Office as per parameter ID_datos column 1
  # check in parameter repo_id_old to see if it was loaded
  offc_loaded <- repo_id_old$office
  check_office = ID_datos$Office[1] %in% offc_loaded # if false load all
  #
  ...
  #
    while (n_row  < top_row & !check_office) { # first load while there are records to be read load them
      ...
      if(check_office == FALSE){ # if the record does not exits process it 
...
archivos <- list.files(path = path_1,recursive = F , 
                       all.files = FALSE, full.names = TRUE, pattern = "*Can*")
...
while (arch_read <= num_arch){
  ...
  # I update it manually for each office need to use TAB 1 information
  # offc_datos_id[1,1] column is "Office"
  # Office to check if office from the record was loaded
  # Canada Records
  #
  # col_ofc_CAN <- repo_id_old[which(repo_id_old$office == "CAN")] 
  # change the variable names ofc_CAN to a proper one like ofc_OFC 
  # code lines 210 to 227
  #
  
  ofc_1 <-  as.character(offc_datos_id[1,1])
  ofc_load <- subset(repo_id_old, office == ofc_1 ) # ofc_load <- offc_datos_id[1,1] column is "Office" use instead of CAN
  col_ofc_load = ofc_load[,"Legacy_ID"] # cual es la columna 6 valor fijo en repoid mejor usar column name
